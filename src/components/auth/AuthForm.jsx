...
export default function AuthForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        if (loggedUser) navigate("/admin/dashboard");
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-100 transform transition-all hover:scale-[1.01]">
      {/* HEADER */}
          ...
          {!isLogin && (
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-900 focus:ring-1 focus:ring-blue-900 transition-all"
                required={!isLogin}
              />
            </div>
          )}
